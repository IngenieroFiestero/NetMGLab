...
function ModuleIdentity(name, lastUpdated, organization, contactInfo, description, revisions, oid) {
    if (!name || name == "") throw new Error("No valid name");
    this.name = name;
 ...
 */
ModuleIdentity.prototype.toString = function toString() {
 ...
 *       ::= { ciscoConfigManMIB 1 }
 */
ObjectIdentifier.prototype.toString = function toString() {
...
function parseObjectType(sequence) {
    ...
    for (i = 0; i < sequence.length; i++) {
        ...
        if (isString) {
            ...
            if (cleared == false && (char == ' ' || char == '\n' || char == '\t')) {
                ...
                } else if (symbol == 'OBJECT-TYPE') {
                    name = lastSymbol;
                }else if (symbol == 'SYNTAX') {
                    var syntx = parseSyntax(sequence.slice(i));
                    syntax = syntx.returned;
                    i += syntx.lastChar;
                } else {
                    switch (lastSymbol) {
                        case 'STATUS':
                ...
                if (char != ' ' && char != '\n' && char != '\t') {
                    if (char == '"') {
                        ...
                        oid.number = Number(lastSymbol);
                        return {'returned' : new ObjectType(name, syntax, maxAccess, status, description, oid), 'lastChar' : i};
                    } else {
                        symbol += char;
                        cleared = false;
                        if (symbol == '--') {//Check if the line is a comment
    ...
    if (sequence.charAt(i - 1) == "}") {
        oid.number = Number(lastSymbol);
        return {'returned' : new ObjectType(name, syntax, maxAccess, status, description, oid), 'lastChar' : i-1};
    }
    throw new Error("Cant parse MODULE IDENTITY: " + name);
}
//----------------------------------------------------------------------------------- TEXTUAL CONVENTION -----------------------------------------------------------------------------------
