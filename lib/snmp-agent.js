...
class SNMPAgent extends EventEmitter {
    ...
    messageRecieved(msg, rinfo) {
        var self = this;
        try {
            var request = new snmp.SNMPPacket(msg, snmp.ENCODING_RULES.BER);
            console.log(request);
            ...
            if (request.community !== this.agent.readOnlyCommunity && request.community !== this.agent.readWriteCommunity) {
                ...
                if (this.agent.device && this.agent.device.ip && this.agent.device.name) {
                    if (this.agent.device.name == 'THIS') {
    ...
    /**
     * Start the SNMPAgent UDP socket
     */
    start() {
        this.socket.bind({
            address: 'localhost',
