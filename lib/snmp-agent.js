 ...
 */
SNMPAgent.prototype.updateAgent = function updateAgent(agent, cb){
    var self = this;
 ...
 */
SNMPAgent.prototype.messageRecieved = function messageRecieved(msg, rinfo){
    var self = this;
    try {
 ...
 */
SNMPAgent.prototype.stop = function stop() {
    var self = this;
    this.socket.close(() => {
        self.emit('stop');
    });
 ...
 * Stop the SNMPAgent UDP socket
 */
SNMPAgent.prototype.start = function start() {
    this.socket.bind({
        address: 'localhost',
